      Subroutine SBCnew      
C +------------------------------------------------------------------------+
C | MAR SURFACE  XF                                        05-10-2010  MAR |
...
C ............x m in 10 layers
     ...
     .         - (0.7512 * (GElatr /  degrad))
      ...
      print *,TypeGL(1:2),mx,my,mz,mw,nsno
...
C +  ======
     ...
     .             *(pstDYn(i,j)+ptopDY)**cap            
       ...
       max_sh      = max(max_sh,sh(i,j))     
      END DO ; END DO 
                
C +--Mask initialisation
C +  ===================
       
      write (*,*)   '1. Reading of MAR_'//TypeGL//'_msk.dat'

      if(TypeGL(1:2) == "GR") then

       open (1, status='old', file='MAR_'//TypeGL//'_msk.dat')
        read(1,'(i4)')   mskSNo 
       close(1)
       ...
       do i=1,mx ; do j=1,my
        if(isolSL(i,j)<=2) mskSNo(i,j) =1
        if(isolSL(i,j)==3) mskSNo(i,j) =3
        if(isolSL(i,j)==4) mskSNo(i,j) =2
       enddo         ; enddo
...
C     ------------
 ...
 150  format(' 3. Snow zone initialisation (',i2,'/',i2,'/',i4,')')
      ...
      if (TypeGL == "GRa") then
       INI = 2
       call CF_READ3D('MARini_sep_1958-2010.nc','RO',1,mx,my,10,ro_ini)
       call CF_READ3D('MARini_sep_1958-2010.nc','TI',1,mx,my,10,ti_ini)
       call CF_READ3D('MARini_sep_1958-2010.nc','G1',1,mx,my,10,g1_ini)
       call CF_READ3D('MARini_sep_1958-2010.nc','G2',1,mx,my,10,g2_ini)
      !call CF_READ2D('MARini_sep_1958-2010.nc','ZN',1,mx,my, 1,zn_ini)
      endif

      if(TypeGL(1:2) == "SV") INI = 3
...
!        endif  
     ...
     .                         rosSNo(i,j,n,k))
         ...
         if(rosSNo(i,j,n,k)>=roCdSV) rosSNo(i,j,n,k)=920
         if(rosSNo(i,j,n,k) <roCdSV) g2sSNo(i,j,n,k)=g2s
...
C +  =================
      ...
      real                    dz_old,zn_old,mb_old,ro_old
...
C +   --------------------
      ...
      do i=1,mx ; do j=1,my ; do n=1,max(1,nsx)

C     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

       if ( zn3IB(i,j,n) > maxlimit .and.mskSNo(i,j) >= 2 ) then
       
        zn_old           = zn0IB (i,j,n)

        do k=1,nssSNo(i,j,n)/3
         dz_old          =                          dzsSNo(i,j,n,k)
         dzsSNo(i,j,n,k) = min(1.,0.75+real(k)/25.)*dzsSNo(i,j,n,k)
...
C     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
       ...
       if ( zn3IB(i,j,n) > 0 .and. zn3IB(i,j,n) < minlimit 
     .                       .and.mskSNo(i,j) >= 3 ) then
        ...
        do k=1,min(3,nssSNo(i,j,n)/4)
         ...
         rosSNo(i,j,n,k) = max(ro_ice,rosSNo(i,j,n,k))
...
C     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
       ...
       do k=-1,1 ; do l=-1,1
        if(mskSNo(i+k,j+l) >= 3                      .and.
     .     rosSNo(i+k,j+l,n,max(1,nssSNo(i+k,j+l,n)))<=800) then
                            ww=1     
         if (k==0 .or.l==0) ww=2
         if (k==0.and.l==0) ww=4
         ...
         al_new = al_new+albeSL(i+k,j+l)                    *ww
...
C     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     ...
     .    albeSL(i,j)                <al_new-0.05 .and.
     .    g2_new                     <50.         .and.
     .    albeSL(i,j)                <0.70        .and.
     .    albeSL(i,j)                >0.50        .and.
     .    g2sSNo(i,j,n,nssSNo(i,j,n))<75.         ) filtering=1

c       if(nbr2==9.and.albeSL(i,j)<al_new-0.1      .and.
...
C     ==================================
      ...
      IF (.not. first .and. ipr_nc_one == 0 ) THEN
       ...
       print *, ' ' 
       print *, 'Begin of OUTone initialisation for '//TypeGL
       print *, '======================================'
       print *, ' '  
