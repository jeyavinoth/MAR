...
!  & MONTH, IDAY, IYR, ILWRAD, IFIN, NMM, NDD, NAA, IMINUT, INHOMF, &
  & MONTH, IDAY, IYR, ILWRAD, IFIN, IMINUT, INHOMF, &
...
REAL_B :: ZMFDSTD,ZMFUSTD,ZMFDSCD,ZMFDSI
REAL_B :: VAR2D(16)
...
if (ISTAT.EQ.0.and.NWSET.eq.0) then
   ...
   ISEND=5
   LMCCLAT=.TRUE.
   WRITE(0,*) 'Enter Emphasis on tropo- (0) or strato- sphere (1)?'
   read (*,8900) ITROSTRA
   LCLOUD=.TRUE.
   LFORCLD=.TRUE.
...
9016  format(I3)
  ...
  WRITE(0,*) 'Enter INHOMF  =1 Tiedtke, 1995  tau x 0.7'
  read (*,9012) INHOMF
!  INHOMF=1
  if (INHOMF.EQ.0) then
    LINHOM=.FALSE.
...
DO IDFIL=90,95
  if (IWRT(IDFIL).EQ.1) then
    write (96,8921) ZLATP,ZLONP,PLAT5(KIDIA),PLON5(KIDIA)
8921 format(1x,2F10.3,2f12.6)
    write (96,8922) CPROF
!    write (96,8922) CSURF
8922 format(A80)
    write (96,8923) NOZOCL,ILCLOUD,IOPTPROP,INHOMF,ILWRAD,NAER,KSW,KBOX,NBOX,IRADLP
...
!        
      ELSE IF (ICLDMIX.EQ.2) THEN
        ...
        PTARG=PT5(JL,JK)
        ZFRACT = MIN( _ONE_ ,((MAX(RTICE,MIN(RTWAT,PTARG))-RTICE)&
     &               /(RTWAT-RTICE))**2) 
...
!            print 9996,JK,ZCOOLC,ZSIGMA
            print 9996,JK,ZCOOLT,ZCOOLC,ZHEATT,ZHEATC,ZSIGMA,ZDPGCP &
