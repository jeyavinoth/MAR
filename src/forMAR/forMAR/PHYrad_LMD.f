      SUBROUTINE ozoncli(rjour, rlat, paprso, o3)
C +
C +--------------------------------------------------------+
C |   SUBROUTINE ozoncli generates a climatology           |
C |              of the  ozone       concentration         |
C |   Source:    Laurent Li         (CNRS, LMD)            |
C +--------------------------------------------------------+
C +
      IMPLICIT none
      include 'MARdim.inc'
      REAL   rlat(klon)
      REAL paprso(klon,klev+1)
      REAL     o3(klon,klev)
...
C----------------------------------------------------------
     ...
     .          +zslat**2
     . *(.056222+zslat**2*(-.037609 +.012248 *zsint
     .                              +.00521  *zcost
     .                              +.008890 *zslat))
      zo3a3 = zo3q3/ps/2.
      z     = z-zo3q3*ps
...
c
      RETURN
      END
      SUBROUTINE radlwsw(dist, rmu0, fract, co2_ppm, solaire,
     .                  paprs, pplay,tsol,albedo, t,q,wo,
     .                  cldfra, cldemi, cldtau, aertau,
...
c Adaptation to MAR: H.Gallee (LTHE/IRD) date: 20001111 20021123
c======================================================================
...
c aertau---input-R- epaisseur optique des aerosols dans le visible
c
...
c======================================================================
C +
C +--BEGIN of MAR addenda
C +  ~~~~~~~~~~~~~~~~~~~~
      include "MARdim.inc"
C +
C +--END   of MAR addenda
C +  ~~~~~~~~~~~~~~~~~~~~
...
c
      ...
      ENDDO
      ENDDO
c Terrigeneous Aerosols v Addendum HG (23 november 2002)
      DO k = 1, kflev
      DO i = 1, kdlon
         PAER(i,k, 1) = MAX(aertau(iof+i,k),1.0E-15)
      ENDDO
      ENDDO
c Terrigeneous Aerosols ^ Addendum HG (23 november 2002)
c
c======================================================================
...
c
      IF (appel1er) THEN
         PRINT*, 'SW            runing  frequency: ', swpas
...
c clear-sky:
     ...
     .         ZAKI,ZCLD,ZCLEAR,ZDSIG,ZFACT,ZRMU,ZSEC,ZUD)
      INU = 1
      CALL SW1S(INU,
     .     PAER, PALBD, PALBP, PCG, ZCLD, ZCLEAR, ZCLDSW0,
     .     ZDSIG, POMEGA, ZOZ, ZRMU, ZSEC, PTAU, ZUD,
     .     ZFD, ZFU)
      INU = 2
      CALL SW2S(INU,
     .     PAER, ZAKI, PALBD, PALBP, PCG, ZCLD, ZCLEAR, ZCLDSW0,
     .     ZDSIG, POMEGA, ZOZ, ZRMU, ZSEC, PTAU, ZUD,
     .     PWV, PQS,
     .     ZFDOWN, ZFUP)
      DO JK = 1 , KFLEV+1
...
c cloudy-sky:
     ...
     .         ZAKI,ZCLD,ZCLEAR,ZDSIG,ZFACT,ZRMU,ZSEC,ZUD)
      INU = 1
      CALL SW1S(INU,
     .     PAER, PALBD, PALBP, PCG, ZCLD, ZCLEAR, PCLDSW,
     .     ZDSIG, POMEGA, ZOZ, ZRMU, ZSEC, PTAU, ZUD,
     .     ZFD, ZFU)
      INU = 2
      CALL SW2S(INU,
     .     PAER, ZAKI, PALBD, PALBP, PCG, ZCLD, ZCLEAR, PCLDSW,
     .     ZDSIG, POMEGA, ZOZ, ZRMU, ZSEC, PTAU, ZUD,
     .     PWV, PQS,
     .    ZFDOWN, ZFUP)
      DO JK = 1 , KFLEV+1
...
C
     ...
     .                     -(ZFSDN(i,k)-ZFSDN(i,kpl1))
            ...
            PHEAT0(i,k) = -(ZFSUP(i,kpl1)-ZFSUP(i,k))
     .                     -(ZFSDN(i,k)-ZFSDN(i,kpl1))
...
C
      CALL SWR ( KNU
     .  , PALBD ,PCG   ,PCLD  ,PDSIG ,POMEGA,ZRAYL
     .  , PSEC  ,PTAU
...
C
      ...
      REAL*8 PRE1(KDLON)  ! LAYER REFLECTIVITY   (NO UNDERLY.-LAYER REFLECTION)
      REAL*8 PRE2(KDLON)  ! LAYER REFLECTIVITY
      REAL*8 PTR1(KDLON)  ! LAYER TRANSMISSIVITY (NO UNDERLY.-LAYER REFLECTION)
...
CMAF      ZARG=MIN(ZTOP/PRMUZ(JL),200.)
      ZARG=MIN(ZTOP/PRMUZ(JL),200.0)
      ZEXMU0=EXP(-ZARG)
CMAF      ZARG2=MIN(ZRK*ZTOP,200.)
      ZARG2=MIN(ZRK*ZTOP,200.0)
      ZEXKP=EXP(ZARG2)
      ZEXKM = 1./ZEXKP
      ZXP2P = 1.+ZDT*ZRP
      ZXM2P = 1.-ZDT*ZRP
...
C     ------------------------------------------------------------------
      IF (appel1er) THEN
         PRINT*, "LW clear--sky runing  frequency: ", lw0pas
         PRINT*, "LW cloudy-sky runing  frequency: ", lwpas
...
C
     ...
     .         PAER,PDP,PPMB,PPSOL,ZOZ,PTAVE,PVIEW,PWV,ZABCU)
...
c #WL DO i = 1, KDLON
c #WL     write(6,6000) i
 6000     format(/,'Longwave Radiative Fluxes  Grid Pt.',i2,
     .           /,'=====================================',
     .           /,'    k |      UP |    DOWN | COOLING |',
     .           /,'------+---------+---------+---------+')
c #WL   DO k=1,KFLEV
c #WL     write(6,6001) k,ZFLUX(i,1,k),ZFLUX(i,2,k),PCOLR(1,k)
 6001     format(i5,' |',2(f8.1,' |'),f8.2,' |')
c #WL   END DO
c #WL END DO
C +
C     ------------------------------------------------------------------
...
CMAF      ZUP=MIN( MAX( 0.5*R10E*LOG( ZZABLY ) + 5., 0.), 6.0)
      ZUP=MIN( MAX( 0.5*R10E*LOG( ZZABLY ) + 5., 0.), 6.)
...
C* PERFORMS THE VERTICAL INTEGRATION:
     ...
     .  , PABCU,ZB,PBINT,PBSUI,ZBSUR,ZBTOP,ZDBSL,PEMIS,PPMB,PTAVE
     .  , ZGA,ZGB,ZGASUR,ZGBSUR,ZGATOP,ZGBTOP
     .  , PCNTRB,PCTS,PFLUC)
C     ------------------------------------------------------------------
      RETURN
...
C
      DO 130 JA = 1 , 8
      DO 120 JL = 1, KDLON
      zzm     =SQRT(PUU1(JL,JA) - PUU2(JL,JA))
      ZXD      =PGB( JL,JA,1) + zzm      *(PGB( JL,JA,2) + zzm      )
      ZXN      =PGA( JL,JA,1) + zzm      *(PGA( JL,JA,2) )
      PTT(JL,JA)=ZXN      /ZXD
  120 CONTINUE
  130 CONTINUE
C
...
C +... Dry air Cv
C +
c #WR  write(6,600) R,RD,RCPD,RCVD
 600   format(/,' From MAR to LMDZ constants',
     .        /,' ==========================',
     .        /,' R      = ',f15.6,
     .        /,' RD     = ',f15.6,
     .        /,' RCPD   = ',f15.6,
