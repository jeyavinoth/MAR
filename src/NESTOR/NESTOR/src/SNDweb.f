C   +-------------------------------------------------------------------+
C   |  Subroutine SNDweb                             June 2002  NESTING |
C   +-------------------------------------------------------------------+
C   |                                                                   |
C   | Input : - SNDfil : sounding file from http://weather.uwyo.edu     |
C   | ^^^^^^^                                                           |
C   |                                                                   |
...
221    format(a60)
       ...
       write(6,*) 'Sounding station: ',SNDsta
       write(6,*)
       write(6,*) 'Pressure   Temp.  Rel.H.   Wind  Direct.'
       write(6,*) '--------   -----  ------   ----  -------'

       i=1
       read  (15,224) SND_pp(i),SND_hgt,SND_tt(i),SND_dwpt,
     .                SND_hr(i),SND_qv(i),SND_dd(i),SND_ff(i)
       SND_qv(i) = 0.001*SND_qv(i)
       write ( 6,225) SND_pp(i),SND_tt(i),SND_hr(i),SND_ff(i),SND_dd(i)
224    format(f7.1,f7.0,2f7.1,f7.0,f7.2,2f7.0)
225    format(1x,5f8.0)
 
       DO WHILE (i.lt.(mzm-4).and.SND_pp(i).ne.0.)
        i=i+1
        read (15,224) SND_pp(i),SND_hgt,SND_tt(i),SND_dwpt,
     .                SND_hr(i),SND_qv(i),SND_dd(i),SND_ff(i)
        SND_hr(i)=MAX(SND_hr(i),1.)
        SND_qv(i)=0.001*SND_qv(i)
        IF (SND_pp(i).ne.0.) THEN
         IF (SND_ff(i).eq.0.) THEN
          SND_ff(i) = SND_ff(i-1)
          SND_dd(i) = SND_dd(i-1)
         ENDIF
         write( 6,225) SND_pp(i),SND_tt(i),SND_hr(i),
     .                 SND_ff(i),SND_dd(i)
        ENDIF
       ENDDO
       ifin=i-1
       ...
       read (15,226) SNDyar,SNDmma,SNDjda,SNDjhu
226    format(45x,3i2,1x,i2)
       IF (SNDyar.lt.50) THEN
        SNDyar = SNDyar + 2000
       ELSE
        SNDyar = SNDyar + 1900
       ENDIF
       write(6,121) SNDjda,SNDmma,SNDyar,SNDjhu
121    format(' Date: ',i2,'/',i2,'/',i4,' at ',i2,'h.')
...
227    format(45x,f7.2)
       ...
       write(6,122) SNDlat,SNDlon
122    format(' Latitude: ',f6.2,' - Longitude: ',f6.2)
...
C +   =================
      ...
      DO i=1,mzm
C +    SND_qv(i)=SND_hr(i)/100.*qsat(SND_tt(i),SND_pp(i)/10.)
...
C +---Update of specific humidity
C +   ===========================

c     DO i=1,ms
c      SND_qv(i)=SND_hr(i)/100.*qsat(SND_tt(i),SND_pp(i))
c      SND_qv(i)=MAX(0.000003,SND_qv(i))
c     ENDDO
